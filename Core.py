...
class ExampleApplicationsCore:
	...
	def LFinference(self, text, s_prev, ref_s, alpha=0.3, beta=0.7, t=0.7,
		...
		return out.squeeze().cpu().numpy()[..., :-100], s_pred  # weird

	def STinference(self,text, ref_s, ref_text, alpha=0.3, beta=0.7, diffusion_steps=5, embedding_scale=1, use_gruut=False):
		text = text.strip()
		ps = global_phonemizer.phonemize([text])
		ps = word_tokenize(ps[0])
		ps = ' '.join(ps)
